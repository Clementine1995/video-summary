...
from .audio import download_video_audio
from .cache import find_resume_dir, load_cached_chunk_summaries, load_cached_pipeline_inputs
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize a YouTube or Bilibili video into Markdown.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if not is_supported_video_url(args.url):
            raise UnsupportedInputError("当前支持 YouTube 和 B站视频链接。本地文件会在后续阶段加入。")
        ...
        cached_chunk_summaries = load_cached_chunk_summaries(output_dir / "chunk_summaries.md") if args.resume else []
